    ...
    **kwargs
    ...
    if output_file is None:
        output_dir = os.path.join(
            os.path.dirname(input_file), "quantized_models")
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(
            output_dir, f"{os.path.basename(input_file).split('.')[0]}_{ftype}.gguf")
    else:
    ...
    if ftype in LLAMA_QUANTIZATION_TYPES:
        ...
        logger.warning(
            f"Provided ftype '{ftype}' not found in LLAMA_QUANTIZATION_TYPES. Using default Q4_0.")
    ...
    if output_tensor_type:
        if output_tensor_type in GGML_TYPES:
            ...
            logger.warning(
                f"Provided output_tensor_type '{output_tensor_type}' not found in GGML_TYPES. Using default COUNT.")
    ...
    if token_embedding_type:
        if token_embedding_type in GGML_TYPES:
            ...
            logger.warning(
                f"Provided token_embedding_type '{token_embedding_type}' not found in GGML_TYPES. Using default COUNT.")
...
def convert_hf_to_quantized_gguf(
    ...
    **kwargs
    ...
    if os.path.isdir(input_path):
        ...
        safetensors_files = [f for f in os.listdir(
            input_path) if f.endswith('.safetensors')]
        if safetensors_files:
            ...
            try:
                ...
                nexa_convert_hf_to_gguf(model=input_path, outfile=str(
                    tmp_file_path.absolute()), outtype=convert_type, **kwargs)

                # Quantize GGUF model
                quantize_model(str(tmp_file_path.absolute()),
                               output_file, ftype, **kwargs)
        ...
        quantize_model(input_file=input_path,
                       output_file=output_file, ftype=ftype, **kwargs)
        return output_file, input_name, ftype
    else:
        logger.error(
            f"Invalid input path: {input_path}. Must be a directory with .safetensors files or a .gguf file.")
        return None


def main():
    parser = argparse.ArgumentParser(
        description="Convert and quantize a Hugging Face model to GGUF format.")
    # nexa convert specific arguments
    parser.add_argument("input_path", type=str,
                        help="Path to the input Hugging Face model directory or GGUF file")
    parser.add_argument("ftype", nargs='?', type=str,
                        default="q4_0", help="Quantization type (default: q4_0)")
    parser.add_argument("output_file", nargs='?', type=str,
                        help="Path to the output quantized GGUF file")
    ...
    parser.add_argument("--convert_type", type=str, default="f16",
                        help="Conversion type for safetensors to GGUF (default: f16)")
    parser.add_argument("--bigendian", action="store_true",
                        help="Use big endian format")
    parser.add_argument("--use_temp_file", action="store_true",
                        help="Use a temporary file during conversion")
    ...
    parser.add_argument("--split_max_tensors", type=int,
                        default=0, help="Maximum number of tensors per split")
    parser.add_argument("--split_max_size", type=str,
                        default="0", help="Maximum size per split")
    parser.add_argument("--no_tensor_first_split",
                        action="store_true", help="Disable tensor-first splitting")
    parser.add_argument("--vocab_only", action="store_true",
                        help="Only process vocabulary")
    ...
    parser.add_argument("--output_tensor_type", type=str,
                        help="Output tensor type")
    parser.add_argument("--token_embedding_type", type=str,
                        help="Token embedding type")
    parser.add_argument("--allow_requantize", action="store_true",
                        help="Allow quantizing non-f32/f16 tensors")
    parser.add_argument("--quantize_output_tensor",
                        action="store_true", help="Quantize output.weight")
    ...
    parser.add_argument("--pure", action="store_true",
                        help="Quantize all tensors to the default type")
    parser.add_argument("--keep_split", action="store_true",
                        help="Quantize to the same number of shards")
    ...
    if args.output_tensor_type:
        kwargs["output_tensor_type"] = GGML_TYPES.get(
            args.output_tensor_type, GGML_TYPE_COUNT)
    if args.token_embedding_type:
        kwargs["token_embedding_type"] = GGML_TYPES.get(
            args.token_embedding_type, GGML_TYPE_COUNT)

    try:
        convert_hf_to_quantized_gguf(
            args.input_path, args.output_file, args.ftype, args.convert_type, **kwargs)
    except Exception as e:
