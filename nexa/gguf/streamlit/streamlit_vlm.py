...
if selected_option == "Local Model 📁":
    ...
    projector_path = st.sidebar.text_input(
        ...
        value=st.session_state.current_projector_path if hasattr(
            st.session_state, 'current_projector_path') else ""
    )
...
if hasattr(st.session_state, 'nexa_model') and st.session_state.nexa_model:
    ...
    if generate_button:
        if not user_input and not uploaded_file:
            st.warning(
                "Please enter text input and upload an image to proceed.")
        else:
