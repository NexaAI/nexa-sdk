...
def parse_nexa_list_output() -> List[str]:
    ...
    try:
        ...
        # filter for Computer Vision models and get their names:
        image_models = set()  # to avoid duplicates
        for match in matches:
            model_name = match[0].strip()
            run_type = match[2].strip()
            if run_type == 'Computer Vision' and not model_name.startswith('Model Name'):  # skip the header row of the `nexa list` output
                image_models.add(model_name)

        return sorted(list(image_models))  # convert to sorted list for consistent ordering
