...
from nexa.eval.nexa_task.group import ConfigurableGroup, GroupConfig
...
class TaskManager:
    ...
    def initialize_tasks(
        ...
        :param include_defaults: bool = True
            If set to false, default tasks (those in nexa.eval/tasks/) are not indexed.
        :return
    ...
    def list_all_tasks(
        ...
        def sanitize_path(path):
            # don't print full path if we are within the nexa.eval/tasks dir !
            # if we aren't though, provide the full path.
            if "nexa.eval/tasks/" in path:
                return "nexa.eval/tasks/" + path.split("nexa.eval/tasks/")[-1]
            else:
                return path
...
def _check_duplicates(task_dict: dict) -> List[str]:
    ...
    Takes the output of nexa.eval.evaluator_utils.get_subtask_list and
