...
from edsl import QuestionMultipleChoice, QuestionFreeText, QuestionLinearScale, Model, Agent, AgentList, ScenarioList, Survey
import sys
...
def example_parameterized_prompts():
    ...
    try:
        ...
        for row in results.select("technology", "interest_level"):
            print(f"Technology: {row['scenario']['technology']:20} -> Interest Level: {row['answer']['interest_level']}")
        print()
