...
ROOT_DIR=$(pwd)
CONFIG_FLAG=Release
LIBS_DEST_DIR=${ROOT_DIR}/nexa/gguf/_libs/cuda
BUILD_DIR=build_gpu
...
build_and_copy_libs "${LLAMA_FOLDER}" "-DGGML_CUDA=ON -DBUILD_SHARED_LIBS=ON" "${LLAMA_LIB_PATHS[@]}"
...
build_and_copy_libs "${SD_FOLDER}" "-DSD_CUBLAS=ON -DSD_BUILD_SHARED_LIBS=ON" "${SD_LIB_PATHS[@]}"