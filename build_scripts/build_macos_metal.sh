...
build_and_copy_libs "${LLAMA_FOLDER}" "-DBUILD_SHARED_LIBS=ON -DGGML_METAL=ON" "${LLAMA_LIB_PATHS[@]}"
...
build_and_copy_libs "${SD_FOLDER}" "-DSD_METAL=ON -DSD_BUILD_SHARED_LIBS=ON" "${SD_LIB_PATHS[@]}"
